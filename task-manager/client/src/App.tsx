...
const CalendarView: React.FC<{
  ...
  const datesWithTasks = new Set(
    allTasks.map((task) => formatDate(new Date(task.date)))
  );
...
const fetchAllTasks = useCallback(async () => {
  ...
  return (
    <div className="h-screen w-screen flex font-sans overflow-hidden min-h-screen">
      {/* --- SIDEBAR --- */}
...
const App: React.FC = () => {
  const { user } = useAuth(); // <-- NO loading here!
  ...
  // --- After login, always show the dashboard! ---
